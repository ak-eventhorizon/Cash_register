...
function changePickForSum(number) {
    ...
    for (let [rate,quantity] of register.content.entries()) {
        ...
        for (let i = quantity; i > 0; i--) {
            ...
            // требуемая сдача равна 0? -- НЕТ
            // требуемая сдача больше текущей купюры?  -- ДА
            // купюра такая есть в кассе?  -- ДА
            // переложить купюру из кассы в сдачу, уменьшить требуемую сдачу на сумму купюры

            // требуемая сдача равна 0? -- НЕТ
            // требуемая сдача больше текущей купюры?  -- НЕТ
            // купюра такая есть в кассе?  -- НЕ ВАЖНО
            // купюры номиналом ниже существуют? -- НЕТ --> возврат бэкапов, статус - нет фондов
            // купюры номиналом ниже существуют? -- ДА --> смена текущей купюры на одну ниже

            // Более подробный алгоритм не забыть забрать с работы!
        }
